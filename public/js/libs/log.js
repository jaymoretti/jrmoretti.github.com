	...
	this.log=function(){
		if(_debugOn){
			...
			if(_stack.indexOf("at") < 0)
			{
				...
				if(_stack.split(" at ")[1].indexOf("(") < -1)
					_file = _stack.split(" at ")[1].split(" (")[1].split(")")[0];
				else
			...
			if(_file.indexOf("anonymous") < 0)
			{
				...
				_filename = _file.replace(/^[^\/]*(?:\/[^\/]*){2}/, "").split(":")[0];
				_class = _stack.split(" at ")[1].split(" (")[0].split("new ")[1];
			...
			var _now = new Date();
			var _stamp = "[ " + _now.getDate() + "/" + (_now.getMonth() + 1) + "/" + _now.getFullYear() + " :: " + (_now.getHours().toString().length == 1 ? "0"+_now.getHours() : _now.getHours() ) + ":" + (_now.getMinutes().toString().length == 1 ? "0"+_now.getMinutes() : _now.getMinutes()) + ":" + _now.getSeconds() + ":" + _now.getMilliseconds() + " :: " + (_isCompiled ? _function : _class) + " at " + _filename + " :: #" + _line + " ]";
			if(__log)
			{
				if(!_isIOS)
				{
