...
function Video(params)/*source, width, height, customFPS, autoplay, outputImageData*/
{
	...
	var filter = null;		// Expects String to be passed via setFilter
	...
	function videoReady()
	{
		...
		scope.currentFrameSet = scope.frames;
		if(scope.video)
			scope.video = null;
	...
	function render( fpos )
	{
		...
		var idata = scope.currentFrameSet[fpos];
	...
	function clone(frames)
	{
		...
		for(var i=0; i!=frames.length; i++)
		{
			...
			tf.push(ctx.getImageData(0,0,idata.width,idata.height));
		}
		return tf;
	}
	...
	{
		_application.worker.postMessage(
			{
				filter: filter.toUpperCase(),
				frames: clone(scope.frames),
				newiData: createImageData(scope.frames[0].width, scope.frames[0].height)
			});
		
		_application.worker.onmessage = function(e)
		{
			scope.processedFrames = [];
			scope.processedFrames = e.data;
			scope.currentFrameSet = scope.processedFrames;
		};
	}
